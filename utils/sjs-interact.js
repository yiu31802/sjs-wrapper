...
function __waitAndGet(params, job_id, app, job, callback, sjsHome){
  ...
  function _pushToCache(job, content){
    ...
    let all_query_objects = cache.map(x => _.omit(x, ["result", "timestamp"]))
    ...
    console.log(cache)
    let result_object = _.extend(_.clone(query_object), {result: content, timestamp: (new Date()).toTimeString()})
